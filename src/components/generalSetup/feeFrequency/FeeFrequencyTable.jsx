...
import {
    useDeleteFeeFrequencyMutation, useEditFeeFrequencyMutation,
    useGetAllFeeFrequencysQuery,
} from "../../../store/features/generalSetup/api.js";
import {updateSnackbar} from "../../../store/snackbar/reducer.js";
import {LinearProgress, ThemeProvider} from "@mui/material";
import themes from "../../reusables/theme.jsx";
import AddFeeFrequencyModal from "./AddFeeFrequencyModal.jsx";

const FeeFrequencyTable = ({searchTerm}) => {
    const {data, isFetching, error} = useGetAllFeeFrequencysQuery()
...
const header = ['S/N', 'Fee Frequency Name', 'Status', 'Actions' ]
...
export function TableData({data, no}) {
    ...
    const [deleteFreq] = useDeleteFeeFrequencyMutation()
    const [editFreq] = useEditFeeFrequencyMutation()
    ...
    const handleOpenView = (data) =>{
        setOpen(true)
        setFeeFreq(data.name)
        setPurpose("view")
        setChecked(data.status === 1 ? true : false )
    }
    const handleRemove = (id)=>{
        deleteFreq(id).then((res) => {
            dispatch(
                updateSnackbar({
    ...
    return (
        <tr>
            ...
            <AddFeeFrequencyModal open={open} setOpen={setOpen} handleAdd={handleEdit} feeFreq={feeFreq} setFeeFreq={setFeeFreq} checked={checked} setChecked={setChecked} purpose={purpose}/>
        </tr>
    )
}