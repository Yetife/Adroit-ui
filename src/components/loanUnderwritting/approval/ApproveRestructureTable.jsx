...
const ApproveRestructureTable = ({searchTerm, applicationId, name, phone, email, channel, startDate, endDate}) => {
    const [page, setPage] = useState(1)
    const [size, setSize] = useState(10)
    const {data, isFetching, error} =  useGetAllApprovalRestructureQuery({
        size,
        page,
...
export default ApproveRestructureTable;
        ...
        <tr>
            ...
            <td className="px-6 py-4 pt-2 text-xs font-medium leading-5 whitespace-no-wrap border-b border-gray-200">
                {permissions.canView && <span
                    ...
                    onClick={() => router(`/loanUnderwriting/restructuring/approval?id=${data.loanApplicationId}&cid=${data.currentLoanApplicationId}&status=approve&type=structure`)}>View
                 </span>}
            </td>
        </tr>
    )
