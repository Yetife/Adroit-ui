...
const DocumentationTable = ({searchTerm}) => {
    ...
    const doc = useSelector((state) => state.documentation.allDoc);
    ...
    const filteredData = doc?.filter(filterData);

    useEffect(() => {
        dispatch(fetchDocumentation(size, page))
    }, []);
...
export function TableData({data, no}) {
    ...
    const download = (e) => {
        fetch(
            "http://prananettech-001-site28.ftempurl.com/Uploads/Client/fd3a14b8-0c55-4e76-88a4-48a71e5884e2_WhatsApp Image 2022-07-18 at 9.26.37 PM.jpeg",
            {
                method: "GET",
                headers: {}
            }
    ...
    return (
        <tr>
            ...
            <td className="px-3 py-4 border-b border-gray-200">
                <span className="text-[16px] leading-5 text-[#4A5D58] font-medium">{data?.lender}</span>
            </td>
            <td className="px-3 py-4 border-b border-gray-200">
